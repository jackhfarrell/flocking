#!/usr/bin/env bash
#SBATCH --job-name=flock_panalyze
...
#SBATCH --output=logs/exponent_precision/%j_analyze.out
#SBATCH --error=logs/exponent_precision/%j_analyze.err
...
julia --startup-file=no --project=. --threads="${JULIA_NUM_THREADS}" \
    ...
    --input-dir "results/exponent_precision" \
    --output-dir "analysis/exponent_precision" \
    --bootstrap "${bootstrap}" \
    --reference-points "${REFERENCE_POINTS:-6}" \
    --minimum-points "${MINIMUM_POINTS:-4}"
