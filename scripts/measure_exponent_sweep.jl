...
# Measure F(r,t) at one temperature, direction, trajectory, and velocity. Each array task
# starts from its matching equilibrated rung, so independent ladder states remain the unit
...
settings = ArgParseSettings(description="Measure one point of the exponent sweep.")
...
vi = mod1(array_id, nv)
trajectory <= args["ntrajectories"] || error("array id exceeds the sweep size")
...
nwindows = round(Int, args["windows-high-v"] +
    (args["windows-low-v"] - args["windows-high-v"]) * fraction^args["budget-power"])
...
let theta = copy(baked.theta)
    for window_index in 1:nwindows
        ...
        @info "measured window" temperature direction trajectory vi v window_index nwindows
    end
end

config = (;
    temperature, J, L, Q, v, vi, direction, trajectory, dt, radii_step=args["dr"],
    r_max=args["r-max"], T_max=args["T-max"], ntimes=args["ntimes"], nwindows,
    seed, solver=string(typeof(solver)), equilibrium=input,
)
